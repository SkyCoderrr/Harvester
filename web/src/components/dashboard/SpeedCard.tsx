...
export function SpeedCard(): JSX.Element {
  // FR-V2-25: refetch dropped from 10s to 60s. Transfer samples are 60s on
  // the server too, so 10s was pure waste.
  const q = useQuery({
    ...
    <section className="bg-bg-sub border border-zinc-800 rounded-lg overflow-hidden">
      ...
      <div className="p-4">
        {q.isLoading ? (
          ...
          <ChartEmpty text="Waiting for the first transfer sample. The probe runs every 60s." />
        ) : (
...
function SpeedChart({
  ...
  // FR-V2-53 memo: recompute only when inputs change.
